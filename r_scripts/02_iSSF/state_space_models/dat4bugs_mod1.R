...
# add following code to restrict nu to be >= 2; required for WinBUGS. not required for JAGS.
# nu.lon[nu.lon < 2] <- 2
# nu.lat[nu.lat < 2] <- 2
...
newdata <- newdata %>% 
  mutate(itau2.lon = case_when(lc =="B" ~ sigma.lon[6],
                               lc =="A"~ sigma.lon[5],
                               lc =="0"~ sigma.lon[4],
                               lc =="1"~ sigma.lon[3],
                               lc =="2"~ sigma.lon[2],
                               lc =="3"~ sigma.lon[1],
                               is.na(lc) ~ NA),
         itau2.lat = case_when(lc =="B" ~ sigma.lat[6],
                               lc =="A"~ sigma.lat[5],
                               lc =="0"~ sigma.lat[4],
                               lc =="1"~ sigma.lat[3],
                               lc =="2"~ sigma.lat[2],
                               lc =="3"~ sigma.lat[1],
                               is.na(lc) ~ NA),
         nu.lon = case_when(lc =="B" ~ nu.lon[6],
                            lc =="A"~ nu.lon[5],
                            lc =="0"~ nu.lon[4],
                            lc =="1"~ nu.lon[3],
                            lc =="2"~ nu.lon[2],
                            lc =="3"~ nu.lon[1],
                            is.na(lc) ~ NA),
         nu.lat = case_when(lc =="B" ~ nu.lat[6],
                            lc =="A"~ nu.lat[5],
                            lc =="0"~ nu.lat[4],
                            lc =="1"~ nu.lat[3],
                            lc =="2"~ nu.lat[2],
...
itau2.lon <- newdata$itau2.lon ^ -2
itau2.lat <- newdata$itau2.lat ^ -2
...
itau2.lon[is.na(itau2.lon)] <- max(itau2.lon, na.rm=T)
...
nu.lon <- newdata$nu.lon
nu.lat <- newdata$nu.lat
