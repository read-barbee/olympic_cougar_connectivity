...
k_fold_inla <- function (dat, form, cov_stack, n_folds, mean_beta = 0, prec_beta = 1e-4 ) {
  ...
  for (i in 1:n_folds) {
    ...
    train <- trn.tst %>% mutate(case_ = case_when(group==i ~ NA,
                                                  .default = case_))
    ...
    mod <- inla(form, family ="Poisson", data= train,
                         ...
                         control.predictor = list(compute = TRUE, link = 1)) ; beepr::beep('fanfare')
    
    # fitted <- mod$summary.fitted.values$mean
    # 
    # plot(train$case_, fitted)
    ...
    for(j in 1:length(names)){
      if(str_detect(names[j], coll("2")) == T){
        ...
        pred[[j]] <- cov_stack_sel[[j]]^2 * coeffs[names[j], "mean"]
      } else{
