#### Skok Detection History Formatting 2021 ####
...
cam_act <- read_csv("data/Camera_Data/2021/Skok_2021/Act_sheet_2021_final.csv") %>% 
...
cam_meta <- read_csv("data/Camera_Data/2021/Skok_2021/skok_cam_metadata_2021_formatted.csv") %>% 
  select(-checked)

cam_act <- cam_act %>% 
  select(-c(lat, lon)) %>% 
  left_join(cam_meta, by=join_by(station_id)) %>% 
  relocate(set_date:lon, .after=camera_id) %>% 
  mutate(set_date = mdy(set_date),
         pull_date = mdy(pull_date)) %>% 
  pivot_longer(cols = -c(station_id:lon), names_to = "date", values_to = "cam_status") %>% 
  mutate(date = mdy(date)) %>% 
  mutate(cam_status = case_when(date < set_date ~ NA,
                                 date > pull_date ~ NA,
                                 .default = cam_status)) %>% 
