...
occ_dat <- read_csv("data/Camera_Data/master/ocp_onp_occ_dat_annual_covs_01-10-24.csv")
...
#write_csv(cor_dat, "feature_selection/pairwise_cov_corr_occ_2_week_01-10-24.csv")
...
cov_names <- covs_scaled %>% 
  select(-c(cell_id, year, aspect)) %>% 
  select(!contains("usfs")) %>% names()

#fit univariate models ~ 2.2 hours for 31 models
uni_fits <- list()
...
cov_stack <- rast("/Users/tb201494/Desktop/annual_cov_stacks_1km_buffer/cov_stack_pred_scaled_01-08-2024.tif")

locs <- read_csv("data/Location_Data/Source_Files/locations_master/gps_locs_dop_screened_10-02-2023.csv")

#test <- occu_cv(uni_fits[[1]], locs=locs, cov_stack = cov_stack)

#function to generate spearman rank correlation for each uni fit
occu_cv <- function(fit, locs, cov_stack){
  # 1.extract coefficient names and values from the fitted model
  int <- summary(fit, submodel = "state")$mean[1]
  b1 <- summary(fit, submodel = "state")$mean[2]
  
  name <- rownames(summary(fit, submodel = "state"))[2]
  
  #import and subset cov stack for prediction
  cov_stack_sel <- cov_stack[[name]]
  ...
  vals <- cov_stack_sel[[1]] %>% terra::setValues(pred_vals) #project those values onto the map
...
#system.time(test <- map(uni_fits, occu_cv, locs=locs, cov_stack = cov_stack))
uni_rho <- list()
for(i in 20:(length(uni_fits) - 1)){
  uni_rho[[i]] <- occu_cv(uni_fits[[i]], locs = locs, cov_stack = cov_stack)
  print(paste0(i, "/", (length(uni_fits) - 1)))
}

uni_rho[[19]] <- NA

uni_rho_flat <- unlist(uni_rho) %>% unname() %>% as_tibble() %>% mutate(model = names(uni_fits)[1:31]) %>% rename(rho = value) %>% select(model, rho)

occu_uni_summ2 <- occu_uni_summ %>% left_join(uni_rho_flat, by=join_by(model))

#write_csv(occu_uni_summ2, "feature_selection/occu_uni_fits_14day_cam_bait_snare_01-10-24.csv")
