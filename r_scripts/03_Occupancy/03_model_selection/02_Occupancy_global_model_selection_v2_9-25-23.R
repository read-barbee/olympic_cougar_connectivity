...
#########################################################################
##
## 1. Import and format occupancy data
##
##########################################################################
...
#fit global model with a random effect for the raster cell in which each camera falls
...
#resample to lower resolution for mapping
cov_stack_resampled <- terra::aggregate(cov_stack, fact=10, cores=5)

#convert from SpatRaster to raster for predict function
...
system.time(surface_rast <- ubms::predict(object = global_fit_cell, 
                   ...
                   transform = TRUE,
                   re.form = NA, #doesn't seem to work if re.form = NULL
                   level = 0.95))
...
#Manually project if the predict function doesn't return a raster
