...
#function to be mapped over each imported OCP detection history
ocp_reframe <- function(dat){
  
  xy <- dat %>% st_as_sf(coords = c("lon", "lat"), crs = 4326) %>% 
  ...
  out <- dat %>% 
    mutate(utm_e = xy[,1],
           utm_n = xy[,2], .after = lat) %>% 
    pivot_longer(cols = -c(station_id:cell_id), names_to = "date", values_to = "cougar_detections_binary") %>% 
    mutate(
      effort_correction_factor = 0,
      effort_correction_factor_bait = 0,
      effort_correction_factor_snare = 0,
      .after = cell_id) %>% 
    mutate(cam_status = case_when(is.na(cougar_detections_binary) ~ NA,
                                  cougar_detections_binary >= 0 ~ 1)) %>% 
    mutate(bait_status = case_when(is.na(cam_status) == T ~ NA,
                                   is.na(cam_status) == F ~ 0),
           snare_status = case_when(is.na(cam_status) == T ~ NA,
                                    is.na(cam_status) == F ~ 0)) %>% 
    select(-station) %>% 
    relocate(cougar_detections_binary, .after = snare_status) %>% 
    mutate(date = ymd(date)) %>% 
    mutate(j_day = yday(date), #convert to julian_day
         .after = date)
...
onp_fisher <- read_csv("data/Camera_Data/Olympic_National_Park/onp_fisher_2013_2016_det_hist_long_10-13-23.csv") %>% 
  mutate(year = year(date), .after = station_id) %>% 
...
#get albers coords for onp data
onp_albers <- onp_fisher %>% 
  st_as_sf(coords=c("utm_e", "utm_n"), crs = 26910) %>% 
  st_transform(crs = 5070) %>% 
  st_coordinates()
...
onp_fisher <- onp_fisher %>% 
  ...
  mutate(utm_e = onp_albers[,1],
         utm_n = onp_albers[,2],
         lon = onp_wgs84[,1],
         lat = onp_wgs84[,2], .before = utm_e) %>% 
  select(-cougar_detections_count)
...
########################################################################
##
## 2. Trim ONP frames for each year to survey period
##
##########################################################################
...
#########################################################################
##
## 4. Batch format OCP data to match ONP format
##
##########################################################################
...
data_list2 <- data_list %>% map(ocp_reframe)
...
#########################################################################
##
## 4. Combine frames and calculate start and end dates for each year
##
##########################################################################
...
dat_final <- bind_rows(data_list_full) %>% 
  select(-contains("effort_correction"))

#summarize start and end dates for each year across all grids
survey_periods <- dat_final %>% 
  bind_rows() %>% 
  group_by(year) %>% 
...
#export
# write_csv(dat_final, "data/Camera_Data/master/ocp_onp_occ_dat_long_11-21-23.csv")
# write_csv(survey_periods, "data/Camera_Data/master/ocp_onp_occ_survey_periods_11-21-23.csv")
