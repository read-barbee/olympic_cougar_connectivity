...
### Working but need to change deployment windows to be non-overlapping***

er_locs_dt <- as.data.table(er_locs_clean)
deployments_dt <- as.data.table(deployments_clean)


loop <- foreach(i=1:n, 
                .combine = 'c', 
                .packages = c('lubridate')) %dopar% {
                dep_select <- deployments_dt[name==er_locs_dt$subject[[i]] & 
                                               er_locs_dt$recorded_at[[i]] %within% interval(start_date, end_date)]
                  if (nrow(dep_select) == 1){
                    return(dep_select[[1,1]])
                  }else if (nrow(dep_select) == 0){
                    return(NA)
...
locs_times <- er_locs_clean %>% 
  mutate(recorded_at_time = format(as.POSIXct(er_locs_clean$recorded_at), 
                                              format = "%H:%M:%S"))

locs_times %>% 
  filter(recorded_at_time == "00:00:00")
