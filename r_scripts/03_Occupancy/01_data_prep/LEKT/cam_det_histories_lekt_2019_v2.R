...
###NOTE: DateTimeOriginal is the time shifted column, NOT dto!


# For counts, detections have to be at least 15 min apart to be considered separate
# 
# Moving camera locations within seasons only matters if it is moved to a different 30m grid cell
...
#camera activity
cam_act <- read_csv("data/Camera_Data/2019/LEKT_2019/2019.Activity.Sheet.csv") %>% 
  select(-c(study, relative_location, camera_id, cam_num:pull_date)) %>% 
  rename(station_id = station)

#species detections
cougar_det <- read_csv("data/Camera_Data/2019/LEKT_2019/SNA_20190703_20200214_2022.09.19_21.23_metadata_tbl.csv") %>% 
  clean_names() %>% 
  mutate(date = mdy(date)) %>% 
  select(station,camera_id, date_time_original, x, y, species) %>% 
