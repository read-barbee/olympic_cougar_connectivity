...
extract_deployments <- function(data, animal_id, collar_id, start_date, end_date) {
  if (is.na(end_date) == TRUE){ #don't filter by end date if it's not included
    trimmed_track <- data %>% 
      ...
      filter(acq_time_utc>= as.POSIXct(start_date)) %>% 
    ...
    trimmed_track <- data %>% 
      filter(collar_id == collar_id) %>% 
      filter(acq_time_utc>= as.POSIXct(start_date) & acq_time_utc<=as.POSIXct(end_date)) %>% 
      mutate(animal_id = animal_id,
             deployment_id = paste(animal_id, "_", collar_id)) %>% 
      select(deployment_id, animal_id, everything())
  }
...
for (i in 1:nrow(cougar_deployments_vec_web)) {
  trimmed_deployments[[i]]<- extract_deployments(
    vec_web_filtered,
    cougar_deployments_vec_web$name[[i]],
    cougar_deployments_vec_web$collar_id[[i]],
    cougar_deployments_vec_web$deployment_date[[i]],
    cougar_deployments_vec_web$end_date[[i]])
}
...
#write_csv(vec_web_formatted, "/Users/tb201494/Library/CloudStorage/Box-Box/olympic_cougar_connectivity/data/Location Data/vec_web_final_2022-10-03.csv")
