...
dispersals <- read_csv("data/Location_Data/Metadata/From_Teams/Formatted_for_R/Dispersals/dispersals_master_reduced_10-02-2023.csv")
...
#note: seems to be flagging dupes that are near in time but on completely different days
...
#remove sex and dispersal columns from individual track objects
locs_nested$tracks <- map(locs_nested$tracks, function(x){x %>% select(-c(sex, dispersal_status))})

locs_nested <- locs_nested %>% select(-c(data, sr)) %>% unnest(cols = c(median_sr)) %>%  relocate(median_sr, .after = dispersal_status)

#remove flagged duplicates from dataframe
# locs_nested_no_dupes <- locs_nested %>% 
#   select(-c(data, sr)) %>% 
#   unnest(cols = c(median_sr)) %>% 
#   unnest(cols=c(tracks)) %>% filter(duplicate_==TRUE)
#   filter(duplicate_==FALSE) %>% 
#   ungroup() %>% 
...
#for loop to fit ctmm model and impute points for each individual (1 hour with 9 cores)
...
####note: simulated frame has 105 fewer observed points than the input locs_screened dataset...
...
mapview::mapview(sims_rerouted[[4]], zcol = "imp_status")
...
#write_csv(sims_rerouted_full_df, "data/Location_Data/imputed_paths/ctmm_sim_imputed_paths_rerouted_12-04-2023.csv" )
