...
#should have 84 individuals remaining after filtering
...
#imputed and rerouted paths
sims_rerouted <- read_csv("data/Location_Data/imputed_paths/ctmm_sim_imputed_paths_rerouted_7-11-2023.csv")
...
#Create sf version of imputed paths for mapping
sims_rerouted_sf <- sims_rerouted %>% st_as_sf(coords=c("x_rerouted", "y_rerouted"), crs = 5070, remove=FALSE, na.fail=FALSE)

#make sure the paths are imputed and rerouted properly
mapview(sims_rerouted_sf %>% filter(animal_id=="Al"),zcol= "imp_status")

#extract wgs84 coordinates to append to df later
wgs84_coords <- sims_rerouted_sf %>% st_transform(crs=4326) %>% st_coordinates()
 
#Reformat imputed path dataframe and join additional metadata for observed points
imp_nested <- sims_rerouted %>% 
  rename(lon_wgs84 = longitude_old,
         lat_wgs84 = latitude_old,
         date_time_utc= timestamp) %>% 
  left_join(locs_screened %>% mutate(date_time_utc = round_date(date_time_utc, unit="hour")), by=join_by(animal_id, lon_wgs84, lat_wgs84, date_time_utc )) %>% #check to make sure join worked
  select(-c(t:lat_wgs84, unique_id, date_time_local:altitude )) %>% 
  mutate(lon_wgs84_rerouted = wgs84_coords[,1],
         lat_wgs84_rerouted = wgs84_coords[,2],
         date_time_local = with_tz(date_time_utc, tzone="US/Pacific")) %>%
  relocate(date_time_local, .after=date_time_utc) %>% 
  relocate(c(lon_wgs84_rerouted, lat_wgs84_rerouted), .after=y_rerouted) %>%
  nest_by(animal_id) %>% 
...
#########################################################################
##
## 4. Create amt tracks from imputed paths
##
##########################################################################

#new version of multitrack function for new variable names
multi_track_rr <- function(d){
  make_track(d, x_rerouted, y_rerouted, date_time_utc, check_duplicates = TRUE, all_cols = TRUE, crs = 5070) 
  #%>% transform_coords(crs_to = 5070)
}

#create tracks
imp_nested$tracks <-map(imp_nested$data, multi_track_rr)

#subset only disperser locations
dispersers <- imp_nested %>% 
  filter(dispersal_status=="disperser")


#########################################################################
##
## 5. Identify dispersal events (only need to do this once and save to csv)
##
##########################################################################
...
#   sf::st_as_sf(coords = c("x_rerouted", "y_rerouted"), crs = 5070) %>% ungroup()
# 
# mapview(filt)
# 
...
imp_nested$sr <-  map(imp_nested$tracks, summarize_sampling_rate)

imp_nested$median_sr <-  map(imp_nested$tracks, function(x){
...
test$steps <- map(imp_nested$tracks, test_fun1)
...
#Remove individuals with < 30steps and/or < 20 days of steps: removes 7 individuals. not sure yet if this is necessary
...
steps_final %>% distinct(animal_id, .keep_all = T) %>% group_by(sex, dispersal_status) %>% count()

#make sure steps look correct
steps_final %>% st_as_sf(coords=c("x2_", "y2_"), crs=5070) %>% filter(animal_id=="Al") %>% mapview(zcol="case_")
...
write_csv(steps_final, "data/Location_Data/Steps/2h_steps_unscaled_imputed_7-12-2023.csv")
