...
steps <- steps %>% 
  mutate(across(c(perc_tree_cov, perc_nonveg, roads_hii_annual), round)) %>% 
...
plot_histogram(steps_filt %>% select(sl_, ta_, elevation:dist_all_roads_annual))
...
plot_correlation(steps_scaled %>% 
                   select(sl_, ta_, elevation:dist_all_roads_annual) %>% 
...
covs <- steps_scaled %>% 
  select(case_, animal_id, step_id_, elevation:dist_all_roads_annual, -aspect) %>%
...
mtpi_fits <- list(mpti_fit, mtpi_fit_quad)
names(mtpi_fits) <- c("mtpi", "mtpi2")
...
#write_csv(mod_table_filt, "feature_selection/uni_muff_summary_no_imp_all_indiv_annual_covs_inla_11-16-23.csv")
