...
locs_screened <- read_csv("data/Location_Data/Source_Files/locations_master/gps_locs_dop_screened_7-3-2023.csv", col_types = list(fix_type = col_character())) %>% 
...
for (i in 1:nrow(locs_nested)){
  locs_nested$tracks[[i]] <-locs_nested$tracks[[i]] %>% flag_duplicates(gamma = minutes(15), DOP = "dop")
}
...
#for loop to fit ctmm model and impute points for each individual 
ctmm_sims <- foreach(i = 1:3, .packages = c("ctmm", "dplyr")) %dopar%{
  ...
  sims_marked <- sims_marked %>% 
    ...
    filter(imp_status =="observed" | n < 12)
...
og_telem_sf <- ctmm_dat[[2]] %>% as_tibble() %>% st_as_sf(coords = c("x", "y"), crs = 5070) %>% 
  mutate(type= "og")

sim_test_sf <- ctmm_sims[[2]] %>% as_tibble() %>% st_as_sf(coords = c("x", "y"), crs = 5070) %>% 
  mutate(type ="sim")

mapview::mapview(bind_rows(sim_test_sf, og_telem_sf), zcol="type")

mapview::mapview(bind_rows(sim_test_sf), zcol="imp_status")
