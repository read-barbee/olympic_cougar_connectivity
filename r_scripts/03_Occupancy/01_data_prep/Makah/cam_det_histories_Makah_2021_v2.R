...
merge_rows <- function(row1, row2){
  if(row1$cell_id==row2$cell_id){
    row_new <- row1[1:5]
    row_new$camera_id <- paste0(row1$camera_id, "_", row2$camera_id)
...
cam_act <- read_csv("data/Camera_Data/2021/Makah_2021/Makah_CameraGrid2021_ForRead/ActivitySheet_FINAL_2.1.22_Number2.csv") %>% 
  select(-c(Study, Elevation, Cameras)) %>% 
...
stations_long <- stations %>%
  pivot_longer(cols=c(-c(station_id:camera_id)), names_to= "date", values_to = "cam_status") %>%
  clean_names() %>%
  mutate(date = dmy(date)) %>% 
  relocate(cell_id, .after=lat) %>% 
  select(-camera_id)
...
#########################################################################
##
## 4. Create cougar detection file from individual camera check csv files
##
##########################################################################
...
cougar_det <- combined_data %>% 
  ...
  select(camera, species, date, time, timestamp) %>% 
...
cougar_det <- cougar_det %>% 
  left_join(cam_to_stat, by=join_by(camera_id)) %>% 
  select(station_id, everything(), -camera_id)


#########################################################################
##
## 5. Generate count of independent cougar detections for each day at each camera station
##
##########################################################################
...
#########################################################################
##
## 6. Join cougar detection counts to activity history by station name and date
##
##########################################################################
...
#########################################################################
##
## 7. Create binary and count detection histories
##
##########################################################################
