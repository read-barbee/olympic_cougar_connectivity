#### Camera Detection Histories LEKT 2020 ####
...
cam_act <-read_csv("data/Camera_Data/2020/LEKT_2020/2020.Camera.Activity.Sheet.IDS.copy.csv") %>%
  select(-c(study, time_cam_on, elevation)) %>% 
...
#species detections
cougar_det <- read_csv("data/Camera_Data/2020/LEKT_2020/S3071_20200513_20201210_2022.12.12_22.30_metadata_tbl.V2.csv") %>% 
  clean_names() %>% 
  select(station, cameras, x, y, date_time_original, species) %>% 
...
stations_long <- stations %>%
  pivot_longer(cols=c(-c(station_id:pull_date)), names_to= "date", values_to = "cam_status") %>%
  clean_names() %>%
