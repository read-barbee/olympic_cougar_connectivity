...
#### NULL MODEL ###
null_mod <- glmmTMB(case_ ~ -1 +
                      (1|step_id_),
                    family = poisson,
                    data = covs, doFit = FALSE)
null_mod$parameters$theta[1] <-log(1e3)
null_mod$mapArg <-list(theta=factor(c(NA)))
null_mod_fit <- glmmTMB::fitTMB(null_mod)

#names(null_mod_fit) <- "null"

#### LINEAR MODELS ###

cov_names <- covs %>% select(-c(case_:step_id_)) %>% names()

#parallelized for loop: fits in 41 seconds
uni_fits <- list()
...
uni_fits_quad <- list()
system.time(uni_fits_quad <- foreach (i = 1:length(cov_names), .packages=c("glmmTMB"))%dopar%{
...
#create summary row for null model
null_mod_row <- tibble(
  name = "null",
  term = "null",
  estimate = NA,
  ...
  statistic = NA,
  p.value = NA,
  AIC = broom.mixed::glance(null_mod_fit)$AIC,
  BIC = broom.mixed::glance(null_mod_fit)$BIC,
  LogLik = broom.mixed::glance(null_mod_fit)$logLik
)

muff_mod_summ <- bind_rows(test_summ, null_mod_row)# %>% filter(!grepl("usfs", name))
...
# write_csv(muff_mod_summ, "feature_selection/uni_muff_summary_quad_all_terms_no_imp_residents_9-18-23.csv")
