...
merge_by_location <- function(data){
  out <- data %>% 
    ...
    summarize(set_date = min(set_date),
              ...
              rows_merged = n(),
              across(-c(camera_id:pull_date), sum_or_na), .groups = "drop") %>%
    ungroup() %>% 
...
write_csv(lekt_dets_all_final, "lekt_detections_all_species_2019-2023.csv")

write_csv(dep_key, "lekt_deployment_key_2019-2023.csv")
