...
inla_fit_quad1 <- run_global_inla(steps_scaled) ;beep("fanfare")
p <- autoplot(inla_fit_quad1)
...
#quads to remove: tri2


inla_fit_quad3 <- run_global_inla(steps_scaled) ;beep("fanfare")
p <- autoplot(inla_fit_quad3)

summary(inla_fit_quad3)


plot_dat <- inla_fit_quad3$summary.fixed %>% rownames_to_column("variable") %>% 
  filter(!(variable %in% c("elevation", "ndvi", "perc_tree_cover", "roads_hii", "tree_cover_hansen")))

plot_dat <- inla_fit_quad3$summary.fixed %>% rownames_to_column("variable") %>% 
  filter(!str_detect(variable, coll("^2")))

ggplot(plot_dat) +
  aes(x = mean, y = variable, colour = as.factor(variable)) +
  geom_point(shape = "circle", size = 1.5) +
  scale_color_hue(direction = 1) +
  geom_errorbarh(aes(xmin = `0.025quant`, xmax = `0.975quant`, height = 0.25)) + # Adjust the width of the error bars
  theme_minimal() +
...
rand_eff <- inla_fit_quad3$summary.random %>% 
  bind_rows(.id = "variable") %>%
  filter(variable=="id1") %>% 
  slice_sample(n=5)

fix_eff <- inla_fit_quad3$summary.fixed %>% 
  rownames_to_column("variable") %>% 
  filter(variable=="tree_cover_hansen") %>% 
  mutate(ID = nrow(rand_eff) + 1, .after = kld)



all_eff <- bind_rows(fix_eff, rand_eff) %>% 
  mutate(eff_type = case_when(str_detect(variable, "id")==T ~ "rand",
                              str_detect(variable, "id")==F ~ "fixed")) %>% 
  arrange(eff_type) %>% 
  mutate(ID = as.factor(c(NA, 1:(nrow(.)-1)))) %>% 
  mutate(ID_name = case_when(eff_type=="fixed" ~ "Global Mean",
                             eff_type=="rand" ~ "Cougar")) %>% 
  unite("ID_name", ID_name, ID) %>% 
  mutate(ID_name = str_remove(ID_name, "_NA")) 

all_eff$ID_name <- factor(all_eff$ID_name, levels = c("Global Mean", "Cougar_1", "Cougar_2", "Cougar_3", "Cougar_4", "Cougar_5"))
  


#example plot of individually estimated slopes and global slope for road impact
ggplot(all_eff) +
  aes(x = mean, y = reorder(ID_name, -as.numeric(ID_name)), colour = as.factor(eff_type)) +
  geom_point(shape = "circle", size = 1.5) +
  scale_color_hue(direction = 1) +
  geom_errorbarh(aes(xmin = `0.025quant`, xmax = `0.975quant`, height = 0.25)) + # Adjust the width of the error bars
  theme_minimal() +
  theme(legend.position = "none") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +  # Add a reference line at 0
  labs(title = "Individual Responses to Canopy Closure with 95% Credible Intervals",
       x = "Mean coefficient estimate", 
       y = "Group level")
...
inla_fit_quad4 <- run_global_inla(steps_scaled) ;beep("fanfare")
p <- autoplot(inla_fit_quad4)

#put roads back in and remove non-significant linear terms
#linear to remove: tri 

inla_fit_quad5 <- run_global_inla(steps_scaled) ;beep("fanfare")
p <- autoplot(inla_fit_quad5)
...
inla_cv <- inla.group.cv(inla_fit_quad5)
...
#aggregate
#cov_stack_capped_r <- terra::aggregate(cov_stack_capped, fact = 2, cores = 5)
...
#writeRaster(binned, "fitted_models/Model_Outputs/inla_fit_quad3_predictions_cropped_60m_10-12-23.tif")

#data exploration

# ranges <- steps %>% filter(case_ ==TRUE) %>% group_by(animal_id) %>% summarize(min_elev = min(elevation, na.rm = T), max_elev = max(elevation, na.rm=T), min_tpi = min(tpi, na.rm=T), max_tpi = max(tpi, na.rm=T)) %>% 
#   mutate(elev_range = max_elev - min_elev, 
#          tpi_range = max_tpi - min_tpi)


########################################################################
##
## 3. Auto stepwise regression
##
##########################################################################

# stack <- INLA::inla.stack(data = list(y = steps_scaled$case_),
#                           A = list(1),
#                           effects = list(data.frame(Intercept = )))
# 
# backwards <- INLAstep(fam1 = "poisson",
#                       dataf = steps_scaled,
#                       spde = NULL,
#                       in_stack = NULL,
#                       invariant)
