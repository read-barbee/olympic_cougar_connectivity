...
retrieved_collars <- cougar_deployments %>% 
  filter(file_source == "Collar") %>%  ###### PROBLEM: fltering out Calypso 87530 because she shares a collar ID with Lilu. Downloaded for lilu and then redeployed
  select(collar_id) %>% 
  unique() %>% 
  unlist() %>% 
  as.vector()
...
test1 <- lotek_complete_raw %>% 
  filter(device_id %in% cougar_deployments$collar_id)

setdiff(lotek_complete_raw$device_id, test1$device_id)

test2 <- lotek_complete_raw %>% 
  filter(!(device_id %in% retrieved_collars))

setdiff(lotek_complete_raw$device_id, test2$device_id)

setdiff(lotek_web_filtered$device_id, cougar_deployments$collar_id)
setdiff(lotek_web_filtered$device_id, lotek_complete_raw$device_id)
setdiff(lotek_complete_raw$device_id, lotek_web_filtered$device_id)
