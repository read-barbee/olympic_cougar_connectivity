...
#########################################################################
##
## 2. Prep data to construct daily camera activity matrix from 2 week interval data
##
##########################################################################
#summarize start and end dates for each year and output for later
survey_periods <- dat %>% 
  group_by(year) %>% 
...
#########################################################################
##
## 3. Convert formatted data to daily activity matrix in camtrapR
##
##########################################################################
...
#########################################################################
##
## 4. Extract raster cell number for each camera station
##
##########################################################################
...
#########################################################################
##
## 5. Format activity matrix and export
##
##########################################################################

#alternatively, import existing activity matrix
# cam_act <- read_csv("data/Camera_Data/Olympic_National_Park/cam_act/onp_fisher_2013_2016_cam_act.csv")
...
#write_csv(cam_act_final, "data/Camera_Data/Olympic_National_Park/cam_act/onp_fisher_2013_2016_cam_act.csv")
...
species_det <- read_csv("data/Camera_Data/Olympic_National_Park/raw/onp_species_det_2013_2016.csv") %>% 
  clean_names()
...
det_hist_corrected <- det_hist_full %>% 
  mutate(cam_status = case_when(cougar_detections_binary > 0 ~ 1,
                                .default = cam_status),
         effort = case_when(cougar_detections_binary > 0 ~ 1,
                                .default = effort))
...
# write_csv(det_hist_full, "data/Camera_Data/Olympic_National_Park/onp_fisher_2013_2016_det_hist_long.csv")
