...
cov_folder_path <- "/Users/tb201494/Desktop/annual_cov_stacks_1km_buffer_water_masked"
...
for (tif_file in cov_files) {
  raster_name <- tools::file_path_sans_ext(basename(tif_file)) %>% str_remove(coll("_water_masked"))
...
#import static stack
static_stack <- rast("/Users/tb201494/Desktop/1km_buffer/static_stack_1km_buffer_water_masked_12-06-23.tif")
...
#static covariates
sf_pts2 <- terra::extract(static_stack, sf_pts, bind = TRUE) %>%  st_as_sf(crs=5070)
...
extract_annual_covs <- function(df, cov_stack_list){
 ...
 for(i in 1:length(df_split)){
  ...
  extract_fun <- function(df_split, year){
    ...
    covs <- terra::extract(stack, vect(df_split), bind = TRUE)
...
#write_csv(occ_dat_all_covs, "data/Camera_Data/master/ocp_onp_occ_dat_annual_covs_12-07-23.csv")
