...
lekt_2020_act <-read_csv("data/Camera_Data/2020/LEKT_2020/2020.Camera.Activity.Sheet.IDS.copy.csv") %>%
...
det_hist_full <- lekt_2020_act %>% 
  pivot_longer(cols=c(`13/05/2020`:`11/12/20`), names_to = "date", values_to = "active") %>% 
  ...
  rename(effort = active,
         longitude = x,
         latitude = y) %>% 
  select(station, camera_id, longitude:date, cougar_detections_count, cougar_detections_binary, effort) %>% 
