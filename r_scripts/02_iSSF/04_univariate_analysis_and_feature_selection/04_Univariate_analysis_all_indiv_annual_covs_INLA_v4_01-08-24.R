...
source("/Users/tb201494/Library/CloudStorage/Box-Box/olympic_cougar_connectivity/r_scripts/02_iSSF/08_cross_validation/k_fold_cv_function_INLA_v3_quad_01-09-24.R")

#cov stack for CV predictions
pred_stack <- rast("/Users/tb201494/Desktop/annual_cov_stacks_1km_buffer/cov_stack_pred_scaled_01-08-2024.tif")
...
#no imputation
steps <- read_csv("data/Location_Data/Steps/2h_steps_unscaled_no_imp_annual_cov_01-08-2024.csv")
...
#write_csv(cor_dat_no_dupes, "feature_selection/pairwise_cov_corr_no_imp_all_indiv_annual_cov_01-08-24.csv")
...
#########################################################################
##
## 7. Prep data for Univariate Muff models (INLA)
##
##########################################################################
#subset dataframe to only include animal_id, case_, and all predictors
covs <- steps_scaled %>% 
  select(case_, animal_id, step_id_, x2_, y2_, elevation:dens_all_roads_annual, -aspect) %>%
...
#null_cv <- k_fold_inla(dat = covs, form = null_form, cov_stack = pred_stack, n_folds = 4)
...
# ~ 11 min #4min per model with 4-fold cross-val
#uni_fits <- list()
...
cv_list <- list()
beta_est <- list()
system.time(for (i  in 1:length(cov_names)) { #19 not enough distinct values in infra for quantile binning
 ...
 cv_list[[i]] <- k_fold_inla(dat = covs, form = form, cov_stack = pred_stack, n_folds = 4, plots = FALSE, seed = 777)
  
  print(paste0(i, "/", length(cov_names)))
 
}) ; beep("fanfare")

# est_tab <- tibble(mod = unlist(mod_names), est = unlist(beta_est))
# 
# mod_tab_full <- read_csv("/Users/tb201494/Library/CloudStorage/Box-Box/olympic_cougar_connectivity/feature_selection/uni_cv_01-09-24.csv") %>% 
#   left_join(est_tab, by = join_by(mod)) %>% relocate(est, .after=mod)
# 
#write_csv(mod_tab_full, "uni_cv_01-08-24.csv")

# cv_list[[19]] <- matrix(c(NA, NA, NA, NA), byrow=FALSE) %>% t()
# 
# cv_list_flat <- map(cv_list, function(x){return(as.data.frame(x))}) %>% bind_rows() %>%
#   mutate(mean_rho = rowMeans(.))
# 
# mod_table <- tibble(mod = unlist(mod_names),
#                     waic = unlist(waic_list),
#                     cpo = unlist(cpo_list)) %>%
#   cbind(cv_list_flat)
# 
# write_csv(mod_table, "uni_cv_last_16_01-08-24.csv")
...
#########################################################################
##
## 13. Examine individual estimates
##
##########################################################################
