...
#system.time(inla_fit_global <- run_global_inla(steps_scaled)) ;beep("fanfare")

p <- autoplot(inla_fit_global)

# res_plot <- ggplot_inla_residuals(inla_fit_global, observed = steps[1:100, "case_"] , binwidth = NULL)

#all linear
#terms: "tree_cover_hansen + ndvi +tpi + northing + tri + perc_tree_cover + roads_hii + popdens_hii + landuse_hii + rails_hii + infra_hii" 
...
#all quad:
#terms: "tree_cover_hansen + ndvi +tpi + northing + tri + perc_tree_cover + roads_hii + popdens_hii + landuse_hii + rails_hii + infra_hii + tree_cover_hansen2 + ndvi2 + tpi2 + northing2  + tri2 + perc_tree_cover2 + roads_hii2 + popdens_hii2 + landuse_hii2"
...
betas <- inla_fit_elev_prob_search3$summary.fixed$mean
...
preds <- exp((betas[1] * cov_stack_pred$tree_cover_hansen) + 
               ...
               (betas[5] * cov_stack_pred$roads_hii) + 
               (betas[6] * cov_stack_pred$popdens_hii) +
               (betas[7] * cov_stack_pred$landuse_hii) + 
               (betas[8] * cov_stack_pred$rails_hii) +
               (betas[9] * cov_stack_pred$infra_hii) +
               (betas[10] * cov_stack_pred$elevation) +
               (betas[11] * cov_stack_pred$tree_cover_hansen^2) +
               #(betas[11] * cov_stack_pred$ndvi^2) +
               #(betas[14] * cov_stack_pred$tpi^2) +
               (betas[12] * cov_stack_pred$northing^2) +
              #(betas[16] * cov_stack_pred$tri^2) +
               (betas[13] * cov_stack_pred$perc_tree_cover^2) +
               (betas[14] * cov_stack_pred$roads_hii^2) +
               (betas[15] * cov_stack_pred$popdens_hii^2) +
               (betas[16] * cov_stack_pred$landuse_hii^2) +
