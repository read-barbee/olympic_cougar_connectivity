...
global <- glmmTMB(case_ ~ -1 +
                    ...
                    (0 +  gpp | animal_id) +
                    (0 +  land_use_usfs_lumped_other | animal_id) +
...
global$mapArg <- list(theta=factor(c(NA, 1:19)))
...
#stepwise--not implemented for glmmTMB
#sel_test <- MASS::stepAIC(fit, direction = "backward")
...
global2$mapArg <- list(theta=factor(c(NA, 1:18)))

system.time(fit <- fitTMB(global2)); beep("fanfare")



#Fit global glmmTMB model: takes about 11 minutes
...
system.time(fit9 <- fitTMB(global3))#; beep("fanfare")

fits <- list(fit, fit2, fit3, fit4, fit5, fit6, fit7, fit8, fit9)
names(fits) <- c("fit", "fit2", "fit3", "fit4", "fit5", "fit6", "fit7", "fit8", "fit9")
AICcmodavg::bictab(fits)
...
##
##########################################################################
vars <- c("tree_cover_hansen + (0 + tree_cover_hansen | animal_id)",
          "gpp + (0 + gpp | animal_id)",
          "northing + (0 + northing | animal_id)",
          "perc_tree_cover + (0 + perc_tree_cover | animal_id)",
          "landuse_hii + (0 + landuse_hii| animal_id)",
          "ndvi + (0 + ndvi | animal_id)",
