...
k_fold_tmb <- function (dat, form, cov_stack, n_folds) {
  ...
  for (i in 1:n_folds) {
    ...
    mod_sum <- summary(mod.fit)
    coeffs <- mod_sum$coefficients$cond %>% as.data.frame()
    names <- rownames(mod_sum$coefficients$cond)
    
    #Generate predictions by multiplying each covariate layer by its coefficient estimate from the model
    pred <- list()
    for(j in 1:length(names)){
      pred[[j]] <- cov_stack[[j]] * coeffs[names[j], "Estimate"]
    }
    ...
    vals <- cov_stack[[1]] %>% terra::setValues(preds_exp) #project those values onto the map
